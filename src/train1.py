# =============================
# Soft Top-K Subsequence Importance 加权 Loss (稳定版 v2)
# =============================
import re
import torch
...
np.save("train_indices_v4.npy", np.array(train_indices))
np.save("val_indices_v4.npy", np.array(val_indices))
...
for epoch in range(n_epochs):
    ...
    for x, importance, mask, y, seq_names in train_loader:
        ...
        seq_imp = soft_topk_importance(importance, k=5, tau=0.7)

        loss_mat = focal_loss(logits, y)
        loss_per_sample = torch.sqrt(loss_mat.sum(dim=1) + 1e-8)
        weighted_loss = loss_per_sample * seq_imp
        loss = weighted_loss.mean()  # ✅ 改成 mean
    ...
    if acc_total > best_val_acc:
        ...
        wait = 0
        torch.save(model.state_dict(), "best_model_softtopk_v4.pth")
...
# 保存日志 & 绘图
# ----------------------------
pd.DataFrame(all_metrics).to_csv("metrics_softtopk_v4.csv", index=False)
...
plt.savefig("loss_curve_softtopk_v4.png", dpi=300)
plt.show()